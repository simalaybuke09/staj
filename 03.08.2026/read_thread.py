...
class ReadThread(QThread):
    ...
    def __init__(self, plc ,tab_name, db, start, size, veri_tipi, bool_index):
        super().__init__()
        self.plc = plc
    ...
    def run(self):
        ...
        if self.tab_name == "Datablock":
            ...
            ham_veri = self.plc.read_datablocks(self.db,self.baslangic_byte,self.size)
            parametreler = Params(byte_array=ham_veri, byte_index=0,size=self.size, data_type=self.veri_tipi, bool_index =self.bool_index)
            ...
            if self.tab_name == "Giriş":
                veri = self.plc.read_inputs(self.start, self.size)
            elif self.tab_name == "Çıkış":
                veri = self.plc.read_outputs(self.start, self.size)
            elif self.tab_name == "Marker":
                veri = self.plc.read_markers(self.start, self.size) 
